...
opt version => (
  isa     => 'Str',
  comment => 'Specify alternate version',
);
...
sub run
{
  ...
  print "NAME ";
  my($name, $version) = find_name_and_version($content, $opt);
  print " $name $version";

  $name     = $opt->{name}    if defined $opt->{name};
  $version  = $opt->{version} if defined $opt->{version};
  ...
  my $root = App::optbrew->root->subdir(lc $name, $version);
...
sub find_name_and_version
{
  ...
  while(archive_read_next_header($a, my $e) == ARCHIVE_OK)
  {
    ...
    {
      if(defined $opt->{name} && defined $opt->{version})
      {
